...
class PublicationProjectImport extends \TYPO3\CMS\Scheduler\Task\AbstractTask {
	...
	$executionSucceeded = FALSE;
        ...
        $query->setQuery('docType:upmproject AND (organisationSourceId:v1000170)');
        //$query->setQuery('id:40b06f29-577e-40eb-9c4a-ac055b660964');
        $query->setStart(0)->setRows(20000);
...
//print "<table border=\"1\">";
        foreach ($response as $document) {
            ...
            $publicationIdArray = $document->publicationId;
            //print "<td>$id</td>";
            $html = @file_get_contents("http://portal.research.lu.se/portal/sv/projects/yada($projectId).html");
            $htmlArray = explode('rendering_researchoutput_short', $html);
            array_shift($htmlArray);
            //print_r($htmlArray);
            foreach ($htmlArray as $key => $value) {
                $projectIdArray = array();
                preg_match('/<a rel=\"(.*?)<\/a>/s', $value, $matches);
                preg_match('#\((.*?)\)#', $matches[1], $matchas);
                //print "<td>" . $matchas[1]  . "</td>";
                $publicationId = $matchas[1];
                //Get publication
                $query1 = $client->createSelect();
                $query1->setQuery("id:$publicationId");
                $query1->setStart(0)->setRows(1);
                $response1 = $client->select($query1);
                foreach ($response1 as $document1) {
                    $projectIdArray = $document1->upmProjectId;
                }
                $projectIdArray[] = $projectId;
                $projectIdArray = array_unique($projectIdArray);
                //Update publication
                $docArray = array();
                $update = $client->createUpdate();
                ${"doc"} = $update->createDocument();
                ${"doc"}->setKey('id', $publicationId);
                ${"doc"}->addField('upmProjectId', $projectIdArray);
                ${"doc"}->setFieldModifier('upmProjectId', 'set');
            ...
            //Update project
            $docArray = array();
            if($publicationIdArray) $publicationIdArray = array_unique($publicationIdArray);
            $update = $client->createUpdate();
            ${"doc"} = $update->createDocument();
            ${"doc"}->setKey('id', $projectId);
            ${"doc"}->addField('publicationId', $publicationIdArray);
            ${"doc"}->setFieldModifier('publicationId', 'set');
            ${"doc"}->addField('appKey', 'lthsolr');
            ${"doc"}->setFieldModifier('appKey', 'set');
            ${"doc"}->addField('type', 'upmproject');
            ${"doc"}->setFieldModifier('type', 'set');
            $docArray[] = ${"doc"};
            $update->addDocuments($docArray);
            $update->addCommit();
